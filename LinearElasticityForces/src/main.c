...
{  
    ...
    geoSetDomainName(3,"HookA");
    geoSetDomainName(4,"HookB");
    geoSetDomainName(5,"HookC");
...
//
    ...
    femElasticityAddBoundaryCondition(theProblem,"HookA",NEUMANN_X,force);
    femElasticityAddBoundaryCondition(theProblem,"HookB",NEUMANN_X,force);
    femElasticityAddBoundaryCondition(theProblem,"HookC",NEUMANN_X,force);
...
//  
    ...
    do {
        ...
        if (glfwGetKey(window,'T') == GLFW_PRESS) { mode = 4;}
        ...
        if (mode == 3) {
            ...
            glColor3f(1.0,0.0,0.0); glfemMessage(theMessage); }
        if (mode == 4) {
            glfemPlotTensionField(theGeometry->theElements,normDisplacement);
            glfemPlotMesh(theGeometry->theElements); 
            sprintf(theMessage, "Number of elements : %d ",theGeometry->theElements->nElem);
            glColor3f(1.0,0.0,0.0); glfemMessage(theMessage); }
