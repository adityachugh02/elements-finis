...
/*
double computeElementVonMisesStress(femNodes *theNodes, int *nodes, double *uElem, int nLocalNode) {

    double E = 210e9;   // Example: steel in Pascals
    double nu = 0.3;

    if (nLocalNode != 3) {
        printf("Only implemented for linear triangular elements.\n");
        exit(1);
    }

    // Node coordinates
    double x0 = theNodes->X[nodes[0]];
    double y0 = theNodes->Y[nodes[0]];
    double x1 = theNodes->X[nodes[1]];
    double y1 = theNodes->Y[nodes[1]];
    double x2 = theNodes->X[nodes[2]];
    double y2 = theNodes->Y[nodes[2]];
    ...
    double beta[3] = { y1 - y2, y2 - y0, y0 - y1 };
    double gamma[3] = { x2 - x1, x0 - x2, x1 - x0 };

    // Extract nodal displacements (u_x, u_y) interleaved in uElem
    double u_x[3] = { uElem[0], uElem[2], uElem[4] };
    double u_y[3] = { uElem[1], uElem[3], uElem[5] };

    // Compute strain components
    double eps_xx = 0.0, eps_yy = 0.0, gamma_xy = 0.0;
    for (int i = 0; i < 3; i++) {
        eps_xx += beta[i] * u_x[i];
        eps_yy += gamma[i] * u_y[i];
        gamma_xy += beta[i] * u_y[i] + gamma[i] * u_x[i];
    }
    eps_xx /= (2 * area);
    eps_yy /= (2 * area);
    gamma_xy /= (2 * area);  // Engineering shear strain

    // Compute stresses using Hooke's Law (Plane Stress)
    double coeff = E / (1.0 - nu * nu);
    double sigma_xx = coeff * (eps_xx + nu * eps_yy);
    double sigma_yy = coeff * (eps_yy + nu * eps_xx);
    double sigma_xy = E / (2 * (1.0 + nu)) * gamma_xy;

    // Compute Von Mises stress
    double vonMises = sqrt(sigma_xx * sigma_xx - sigma_xx * sigma_yy + sigma_yy * sigma_yy + 3.0 * sigma_xy * sigma_xy);

    return vonMises;
}
*/

double computeElementTension(femNodes *theNodes, int *nodes, double *uElem, int nLocalNode) {
    if (nLocalNode != 3) {
        printf("Only implemented for linear triangular elements.\n");
        exit(1);
    }
    ...
    double area = 0.5 * ((x1 - x0) * (y2 - y0) - (x2 - x0) * (y1 - y0));
    if (fabs(area) < 1e-12) {
        printf("Degenerate triangle with near-zero area.\n");
        exit(1);
    }
    ...
    // Compute displacement gradient (strain proxy)
    double du_dx = 0.0, du_dy = 0.0;
    for (int i = 0; i < 3; i++) {
        du_dx += uElem[i] * beta[i];
        du_dy += uElem[i] * gamma[i];
    }
    du_dx /= (2 * area);
    du_dy /= (2 * area);

    // Tension as the norm of the displacement gradient
    double tension = sqrt(du_dx * du_dx + du_dy * du_dy);
    return tension;
}
...
void glfemPlotTensionField(femMesh *theMesh, double *u) {
    ...
    // First: compute tension for each element
    double *tensions = (double *)malloc(nElem * sizeof(double));
    double tensionMin = 1e12, tensionMax = -1e12;
    ...
    for (int i = 0; i < nElem; ++i) {
        ...
        double tension = computeElementTension(theNodes, nodes, uElem, nLocalNode);
        tensions[i] = tension;

        if (tension < tensionMin) tensionMin = tension;
        if (tension > tensionMax) tensionMax = tension;
    }

    // Second: plot the color field based on computed tensions
    for (int i = 0; i < nElem; ++i) {
        nodes = &(theMesh->elem[i * nLocalNode]);
        double scaledTension = glScale(tensionMin, tensionMax, tensions[i]);
        //scaledTension = pow(scaledTension, 2);

        for (int j = 0; j < nLocalNode; ++j) {
            xLoc[j] = theNodes->X[nodes[j]];
            yLoc[j] = theNodes->Y[nodes[j]];
            uLoc[j] = scaledTension;  // Uniform color per element
        }
